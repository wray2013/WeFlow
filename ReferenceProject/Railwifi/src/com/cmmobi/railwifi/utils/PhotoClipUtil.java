...
import com.cmmobi.common.utils.Constant;
...
public class PhotoClipUtil implements OnTouchListener, Callback {
	...
	public PhotoClipUtil(Activity activity,ImageView photoView,ImageView clipView,String picPath,float scale) {
		...
		sideLength = DisplayUtil.getSize(mActivity, 652);
...
//		clipView.setImageBitmap(bmp);
		...
		vto1.addOnGlobalLayoutListener(new OnGlobalLayoutListener() { 
			...
			public void onGlobalLayout() {
				...
				leftEdge = (int) (PhotoClipUtil.this.clipView.getMeasuredWidth() - sideLength) / 2;
				rightEdge = (int) leftEdge + sideLength;
				topEdge = (int) (PhotoClipUtil.this.clipView.getMeasuredHeight() - sideLength) / 2;
				bottomEdge = (int) topEdge + sideLength;
				
				isClipViewShow = true;
				
				PromptDialog.showProgressDialog(mActivity);
...
//				preTranslate();
	...
	private Bitmap createClipBmp(float scale) {
		int length = DisplayUtil.getSize(mActivity, 652);
		Bitmap bmp = Bitmap.createBitmap(length, length, Config.ARGB_8888);
...
//		final Rect rect = clipView.getDrawable().getBounds(); 
		...
		originBitmap = ZGraphics.rotate(originBitmap, ZGraphics.getExifOrientation(picPath), true);
		if (originBitmap == null) {
			PromptDialog.Alert(mActivity, "图片不存在");
			mActivity.finish();
		}
...
//				Rect rect = clipView.getDrawable().getBounds(); 
				System.gc();
				Bitmap originBitmap = BitmapUtils.readBitmapAutoSize(picPath, dm.widthPixels, dm.heightPixels);
				int angle = ZGraphics.getExifOrientation(picPath);
...
//				this.start();
	...
	public void resetPic(Bitmap bmp) {
		...
		leftEdge = (int) (clipView.getMeasuredWidth() - sideLength) / 2;
		rightEdge = (int) leftEdge + sideLength;
		topEdge = (int) (clipView.getMeasuredHeight() - sideLength) / 2;
		bottomEdge = (int) topEdge + sideLength;
	...
	public String saveClipImage(boolean flag) {
		...
		String outputFilePath = ConStant.getPicAbsoluteDirPath()  + imageID + ".jpg" ;
	...
	public String saveClipImage() {
		...
		ZGraphics.setExifOrientation(outputFilePath, angle);
