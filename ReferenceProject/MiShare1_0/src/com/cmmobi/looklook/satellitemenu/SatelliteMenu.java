package com.cmmobi.looklook.satellitemenu;
 ...
 * 
 */
public class SatelliteMenu extends FrameLayout implements OnLongClickListener,OnTouchListener {
	...
	private onFeatureClicklistener itemClickedListener;
	private onFeatureLongClickListener itemLongClickListener;
	private onFeatureToucherListener itemToucherListener;
	...
	public void onClick() {
		Log.d("satelliteMenu onClick", "isEnable="+isEnable);
		//Log.d("satelliteMenu onClick", "plusAnimationActive.compareAndSet(false, true)="+plusAnimationActive.compareAndSet(false, true));
		if(isEnable)return;
		if (plusAnimationActive.compareAndSet(false, true)) {
			Log.d("satelliteMenu onClick", "rotated="+rotated);
			if (!rotated) {
	...
	public void addItems(List<SatelliteMenuItem> items) {
		...
		for (SatelliteMenuItem menuItem : menuItems) {
			...
			cloneView.setOnTouchListener(this);
			cloneView.setOnLongClickListener(this);
	 ...
	 * 禁用+号按钮和菜单点击事件
	 */
	public void setEnableMenuClick(boolean isEnable){
		this.isEnable=isEnable;
	}

	@Override
	public boolean onTouch(View v, MotionEvent event) {
		if(itemToucherListener!=null)
			itemToucherListener.onTouch(Integer.parseInt(v.getTag().toString()), v, event);
		return false;
	}
	...
	public boolean onLongClick(View v) {
		if(itemLongClickListener!=null)
			itemLongClickListener.onLongClick(Integer.parseInt(v.getTag().toString()), v);
		return true;
	}
	...
	private static class SatelliteItemClickAnimationListener implements Animation.AnimationListener {
		...
		@Override
		public void onAnimationEnd(Animation animation) {
			final SatelliteMenu menu = menuRef.get();
	...
	private static class InternalSatelliteOnClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			if(System.currentTimeMillis()-menu.lastClickTime>1000){
				if(menu != null){
					if(menu.isEnable)return;
					SatelliteMenuItem menuItem = menu.getViewToItemMap().get(v);
					v.startAnimation(menuItem.getClickAnimation());	
				}
			}
			menu.lastClickTime=System.currentTimeMillis();
		}
	}
	 ...
	 */
	public void setOnItemClickedListener(onFeatureClicklistener itemClickedListener) {
		this.itemClickedListener = itemClickedListener;
	}
	
	public void setOnItemTouchListener(onFeatureToucherListener itemToucherListener) {
		this.itemToucherListener = itemToucherListener;
	}
	
	public void setOnItemLongClickListener(onFeatureLongClickListener itemLongClickListener) {
	...
	public interface onFeatureLongClickListener {
		void onLongClick(int id,View view);
	}
	
	public interface onFeatureToucherListener {
		void onTouch(int id,View view, MotionEvent event);
	}
