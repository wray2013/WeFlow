...
public class HttpProxy2 {
	...
	private static HttpProxy2 httpproxy;
	...
	//获取单例
	public static HttpProxy2 getInstance() {
		if(httpproxy == null) {
			Log.d(TAG, "Http Proxy Building");
			httpproxy = new HttpProxy2();
		}
		return httpproxy;
	}
...
//			mBufferFileMaximum = maximum;
	...
	public void restartDownload() {
		if (downloadThread == null || !downloadThread.isDownloading()) {
			...
			Log.i(TAG, "----restartDownload:" + mMediaFilePath);
		}
	}
...
//	private boolean isInFileRange(String filename, long range) {
//		File file = new File(filename);
//		boolean ret = false;
//		if(file.exists() && range >= 0) {
//			ret = file.length() >= range;
//		}
//		return ret;
//	}
	/**
...
//			byte[] remote_reply = new byte[1024];
			...
			try {
				...
				if (isExists && request != null) {// MediaPlayer的request有效
					// 需要传http header
					...
					while ((sentBufferSize = utils.sendPrebufferToMP(
							mMediaFilePath, request._rangePosition)) <= 0) {
						Thread.sleep(2000);
						// Log.d(TAG, "(" + sentBufferSize + ")等待缓冲 [ " + new
						// File(mMediaFilePath).length() * 100 /
						// request._rangePosition + "% ]");
						if (request._rangePosition > 0) {
							Log.d(TAG, "(" + sentBufferSize + ")等待缓冲 [ " + downloadThread.getDownloadedSize() * 100 / request._rangePosition + "% ]");
						}
						restartDownload();
					}
					Log.d(TAG, ".................. play over! ..................");
