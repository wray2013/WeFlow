...
import com.cmmobi.looklook.httpproxy.ProxyTempFile.tmpBlock;
				...
				if (request != null) {// MediaPlayer的request有效
					...
					if(tb != null) {// 需要发送缓存到MediaPlayer
						...
						if (!sentResponseHeader) {
							utils.fadeResponseHeader(mBufferSize, request._rangePosition, httpParser);
							sentResponseHeader = true;
						}
						isExists = false;
						int sentBufferSize = 0;
						sentBufferSize = utils.sendPrebufferToMP2(mMediaFilePath, request._rangePosition, (tb.range + tb.size - request._rangePosition));
