...
public class VideoCoverUploaderThread extends Thread implements Callback {
	...
	private VideoCoverUploaderInfo inputParam;
	...
	public VideoCoverUploaderThread(VideoCoverUploaderInfo params) {
		retryTimes = 3;
		uploadedSize = 0;
		this.inputParam = params;
		inputParam.mfile = new File(inputParam.mFilePath);
		...
		try {
			...
			case 2:
				...
				ZLog.e("File: " + inputParam.mfile.getAbsolutePath()
						+ " Already Uploaded.");
//				picUrl = getStringValue(line, "picurl");
//				if(uploadListener!=null)uploadListener.uploadComplete(this);
				...
				OnPercentChangedListener listener = new OnPercentChangedListener() {
					...
					public void onPercentChanged(ZPercent percent) {
						try {
							...
							if(uploadListener!=null)uploadListener.sendException(VideoCoverUploaderThread.this);
							e.printStackTrace();
							if (reader != null) {
								reader.stop();
							}
				...
				if (reader.isEnding()) {
					...
					if (dataover == 2) {
						...
						if (dataover2 == 3) {
							...
							Log.d(TAG,"line3 = " + line3 + " picUrl = " + picUrl);
							if(uploadListener!=null)uploadListener.uploadComplete(this);
							return;
						}
					} else {
	...
	private VideoCoverUploadListener uploadListener;
	public void setUploadListener(VideoCoverUploadListener uploadListener){
		this.uploadListener=uploadListener;
	}
	
	public interface VideoCoverUploadListener{
		void connectException(VideoCoverUploaderThread uploadThread);
		void sendException(VideoCoverUploaderThread uploadThread);
		void uploadComplete(VideoCoverUploaderThread uploadThread);
	}
	...
	public boolean handleMessage(Message msg) {
		...
		case Requester3.RESPONSE_TYPE_GET_SOCKET:
			...
			if (res != null) {
				...
				inputParam.port = Integer.valueOf(res.port);
			}
