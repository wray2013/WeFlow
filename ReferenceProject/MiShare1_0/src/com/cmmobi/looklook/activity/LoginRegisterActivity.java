...
*/				}else{
    ...
    private TextWatcher edit_nick_listener = new TextWatcher(){
		...
		public void afterTextChanged(Editable s) {
			...
			if(a!=null && !a.equals("")){
				if(Prompt.checkUserName(a) && a.length()>=3){
    ...
    private void ShowYZM(boolean show){
    	if(show){
    		...
    		btn_yzm_text.setVisibility(View.GONE);
    		TickDownHelper.getInstance(getHandler()).stop(0);
    	}else{
    		rl_login_reg_yzm.setVisibility(View.GONE);
    		tv_login_yzm_note.setVisibility(View.GONE);
		...
		public void onFocusChange(View v, boolean hasFocus) {
			...
			if(!hasFocus){
				if(ed_nick.getText().toString()!=null && !("".equals(ed_nick.getText().toString()))){
					Requester3.checkNickNameExist(getHandler(), ed_nick.getText().toString());
				}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ll_login_reg = finder.findRelativeLayout(R.id.ll_login_reg);
		...
		btn_yzm_text = finder.findButton(R.id.btn_login_reg_yzm_text);
		...
		btn_yzm_text.setOnClickListener(this);
...
/*				btn_yzm.setEnabled(true);
		...
		case HANDLER_FLAG_ENABLE_YZM:
			if(isPhoneNum){
				...
				btn_yzm_text.setVisibility(View.VISIBLE);
				btn_yzm_text.setEnabled(false);
				ed_yzm.setEnabled(true);
...
/*		case HANDLER_FLAG_YZM_TICK:
		...
		case TickDownHelper.HANDLER_FLAG_TICK_DOWN:
			...
			if(tick==0){
				//btn_yzm.setText("");
				//btn_yzm.setBackgroundResource(R.drawable.btn_activity_login_looklook_yzm);
				btn_yzm_text.setText(R.string.str_cxfs);
				...
				btn_yzm_text.setText("重新发送( " + tick + "秒)");
				//btn_yzm.setBackgroundResource(R.drawable.hqyzm_wait);
...
//						ZLog.e("finish LoginRegisterActivity");
			...
			break;
		case Requester3.RESPONSE_TYPE_LOGIN: // 登陆返回信息
			try {
	...
	public void onClick(View v) {
		...
		case R.id.btn_login_reg_yzm:
			Requester3.getCheckNo(getHandler(), ed_mail_phone.getText().toString(), "1");
/*			ZDialog.show(R.layout.progressdialog, false, true, this);*/
			btn_yzm.setEnabled(false);
			btn_yzm_text.setEnabled(false);
			ed_yzm.setEnabled(false);
			break;
		case R.id.btn_login_reg_yzm_text:
			Requester3.getCheckNo(getHandler(), ed_mail_phone.getText().toString(), "1");
			/*			ZDialog.show(R.layout.progressdialog, false, true, this);*/
			btn_yzm.setEnabled(false);
			btn_yzm_text.setEnabled(false);
...
/*			if (!mbDisplayFlg) {
	...
	private boolean checkValid(String input_nickname, String input_mail_phone, String input_passwd) {
		...
		if(!(/*Prompt.checkEmail(input_mail_phone)||*/Prompt.checkPhoneNum(input_mail_phone))){
			...
			Prompt.Dialog(LoginRegisterActivity.this, false, "提醒", "请输入合法的手机号", null);
			return false;
		}
