...
/*		AlertDialog alertDialog =  new AlertDialog.Builder(this)
		...
		if (userid != null) {
			...
			finder.setOnClickListener(R.id.btn_activity_login_main_qq, this);
	...
	public boolean handleMessage(Message msg) {
		...
		case HANDLER_TENCENT_AUTHOR_SUCCESS:// tencent授权成功，获取微博信息并写入账号，开启loading，开始登陆;
			...
			break;
		case HANDLER_QQMOBILE_AUTHOR_SUCCESS:// 手机qq授权成功 
			ZDialog.show(R.layout.progressdialog, false, true, this);
			acct.snstype = "13";  
			if(acct.snsid!=null){
				QQMobileAuth.getQQMobileAccInfo(getHandler(), acct.snsid, HANDLER_QQMOBILE_ACCOUTINFO_SUCCESS);
			}else{
				acct.logout();
				ZDialog.dismiss();
				//Prompt.Alert(this, "授权微博异常");
			...
			break;
		case HANDLER_QQMOBILE_ACCOUTINFO_SUCCESS:// 获取手机qq个人信息
			...
			if(accInfo==null || !acct.updateQQMobileAuthor(accInfo)){	
				...
				if(CommonInfo.getInstance().equipmentid==null){
					...
					Requester3.login(this, getHandler(), acct, null);
				}
			}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.btn_activity_login_main_qq:

			QQMobileAuth.getInstance(this).authorize(this, new QQMobleUiListener() {
				@Override
				public void qqMobleComplete() {
					...
					Message message = getHandler().obtainMessage(
							HANDLER_QQMOBILE_AUTHOR_SUCCESS);
					getHandler().sendMessage(message);
				}
			});
...
//	        QQMobileAuth.getInstance(this).onActivityResult(requestCode, resultCode, intent);
		}
	}
