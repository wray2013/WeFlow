 ...
 *******************************************************************************/
package com.cmmobi.looklook.common.view.pulltorefresh;
...
//	static final boolean USE_HW_LAYERS = false;
	...
	private boolean mPullDownSilenceEnabled = false;
	...
	private OnHeaderScrolledListener<T> mOnHeaderScrolledListener;
...
//		return false;
	...
	protected boolean isPullDownSilenceEnabled() {
		boolean isEnabled = false;
		if(mPullDownSilenceEnabled) {
			isEnabled = mCurrentMode != Mode.PULL_FROM_END;
		}
		return isEnabled;
	}
	@Override
	...
	public final void setPullDownSilenceEnabled(boolean pullDownSilence) {
		mPullDownSilenceEnabled = pullDownSilence;
	}
	...
	public void setOnHeaderScrolledListener(OnHeaderScrolledListener<T> listener) {
		mOnHeaderScrolledListener = listener;
	}
	 ...
	 * @param hasData
	 */
	public void setNoMoreData(Context context, boolean hasData) {
		mFooterLayout.setNoMoreData(hasData);
		if(hasData) {
			setRefreshingLabel(context.getString(R.string.pull_to_refresh_refreshing_label), Mode.PULL_FROM_END);
			setReleaseLabel(context.getString(R.string.pull_to_refresh_footer_release_label), Mode.PULL_FROM_END);
//			mFooterLayout.setSubHeaderTextVisible(View.VISIBLE);
//			mFooterLayout.setHeaderTextColor(context.getResources().getColor(R.color.default_gray));
//			mFooterLayout.setLastUpdatedLabel("更新于 " + DateUtils.dateToString(new Date(System.currentTimeMillis()), DateUtils.DATE_FORMAT_SHORT));
		} else {
			setRefreshingLabel(context.getString(R.string.no_more_date), Mode.PULL_FROM_END);
			setReleaseLabel(context.getString(R.string.no_more_date), Mode.PULL_FROM_END);
//			mFooterLayout.setSubHeaderTextVisible(View.GONE);
//			mFooterLayout.setLastUpdatedLabel("");
	...
	private Boolean isVshare = false;
	public void setIsVshare(){
		isVshare = true;
	}
...
*/
	...
	protected final void setHeaderScroll(int value) {
		...
		if(false && mState == State.REFRESHING && isPullDownSilenceEnabled()) {
			mOldScrolled = Math.max(Math.abs(value), 50);
		} else {
			mOldScrolled = value;
		}
		...
		if(null != mOnHeaderScrolledListener) {
			mOnHeaderScrolledListener.onHeaderScrolled(this, value, mCurrentMode, mState);
		}
		...
		if (DEBUG) {
			Log.d(LOG_TAG, "setHeaderScroll value: " + value);
		}
		
		if (mLayoutVisibilityChangesEnabled) {
			if (value < 0) {
				if(isPullDownSilenceEnabled()) {
...
//				scrollTo(0, value);
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis,
		...
		switch (getPullToRefreshScrollDirection()) {
			...
			case VERTICAL:
			default:
				oldScrollValue = getScrollY();//isPullDownSilenceEnabled() ? mOldScrolled : getScrollY();
				break;
		}

		if (oldScrollValue != newScrollValue || newScrollValue == 0) {
		...
		/*if(newScrollValue == 0) {
			if(null != mOnHeaderScrolledListener) {
				mOnHeaderScrolledListener.onHeaderScrolled(this, 0, mCurrentMode);
			}
		}*/
	...
	public static interface OnHeaderScrolledListener<V extends View> {
		public void onHeaderScrolled(final PullToRefreshBase<V> refreshView, int value, Mode direction, State state);
	}
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				 ...
				 */
				long rate = 1000;
				long normalizedTime = (rate * (System.currentTimeMillis() - mStartTime)) / mDuration;
				normalizedTime = Math.max(Math.min(normalizedTime, rate), 0);
...
//			Log.e(LOG_TAG, "mScrollToY = " + mScrollToY + ";mCurrentY = " + mCurrentY + ";continued? = " + mContinueRunning);
