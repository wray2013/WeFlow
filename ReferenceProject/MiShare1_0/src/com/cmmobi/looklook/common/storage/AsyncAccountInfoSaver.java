...
public class AsyncAccountInfoSaver extends Thread implements Callback{
	...
	private final int HANDLER_FLAG_ASYNCSAVE_DONE = 0x18791712;
	private final int HANDLER_FLAG_SAVE_DELAY = 0x5381519;
	private transient final long TIME_DELAY_TO_SAVE = 50; //50 毫秒
	...
	public void run(){
		while(true){
			if(write_time<requset_time){
				Log.e(TAG, "AsyncAccountInfoSaver - run, key: " + key + " persist!");
				int tryCount = 3;
				while(tryCount>0){
					try{
