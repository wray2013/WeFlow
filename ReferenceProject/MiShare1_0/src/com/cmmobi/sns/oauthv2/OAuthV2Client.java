...
//    }
     ...
     * @return
     */
     public  static boolean parseAccessTokenAndOpenId(Context context, String responseDataURLEncode, OAuthV2 oAuth){
         /* 腾讯微博
         ...
         try {   
             responseData=URLDecoder.decode(responseDataURLEncode,"utf-8");     
         } catch (UnsupportedEncodingException e) {   
             // TODO Auto-generated catch block   
             e.printStackTrace();   
         }   
    	 Log.i(TAG, "parseAccessTokenAndOpenId - responseData: " + responseData);
...
/*         if (tokenArray.length < 4) {
         ...
         if(curWeiboIndex == SHARE_TO.SINA.ordinal()){  //新浪微博
           ...
           // ---sina weibo, strAccessToken--strExpiresIn---strOpenidaccess_token=2.00jgYbDD8tBWPCea304e3ac4o5uJ8Cexpires_in=157610118uid=2801678985
        	 ...
        	 strExpiresIn = tokenArray[1];
        	 strOpenid = strAccessToken.replace('=', '_').replace('-', '=');
         }
...
/*         ActiveAccount acct = ActiveAccount.getInstance(context);
         if(acct.getLookLookID()==null){
        	 //not login, save it to account
        	 acct.snsid = oAuth.getOpenid();
        	 ...
        	 //after login, don't modify account
         }*/
...
/*        acct.snsid = oAuth.getOpenid();
        ...
        String snstype = acct.snstype;
        String uid = snstype + "_" + acct.snsid;*/
        
       // if(userid!=null && snstype!=null){
        	//登陆后进行绑定，可以持久化
