...
public class ImageLoader {
	 ...
	 */
	public void displayImage(String uri, ImageView imageView, String uid, int belong) {
		displayImage(uri, imageView, null, null, uid, belong);
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(String uri, ImageView imageView, DisplayImageOptions options, String uid, int belong) {
		displayImage(uri, imageView, options, null, uid, belong);
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(String uri, ImageView imageView, ImageLoadingListener listener, String uid, int belong) {
		displayImage(uri, imageView, null, listener, uid, belong);
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(String uri, ImageView imageView, DisplayImageOptions options, ImageLoadingListener listener, String uid, int belong) {
		if(!isInited()){
			Context c = MainApplication.getAppInstance();
			if(uid!=null){
				UniversalImageLoader.initImageLoader(c, uid);
			}
		}
		...
		if (bmp != null && !bmp.isRecycled()) {
		...
		} else if (options.getCacheMemoryKey() != null) {
			Bitmap bm = configuration.memoryCache.get(options.getCacheMemoryKey());
			...
			ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageView, targetSize, memoryCacheKey, options, listener, engine.getLockForUri(uri));
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(engine, imageLoadingInfo, options.getHandler(), uid, belong);
			engine.submit(displayTask);
		} else {
	...
	public void displayImageEx(String uri, ImageView imageView, DisplayImageOptions options, ImageLoadingListener listener, String uid, int belong) {
		//先检查uri是否能在mediamapping中成功hit
		//
		//displayImage(uri, imageView, options, listener, uid, belong);
		if(!isInited()){
			Context c = MainApplication.getAppInstance();
			if(uid!=null){
				UniversalImageLoader.initImageLoader(c, uid);
		...
		if(uri!=null){
			if(uri.startsWith("http://") || uri.startsWith("file://")){
		        ...
		        if(MediaValue.checkMediaAvailable(mv)){//load local
		        	String uri_tmp = "file://" + Environment.getExternalStorageDirectory()  + mv.localpath;
					displayImage(uri_tmp, imageView, options, listener, uid, belong);
		        } 
			}
		}
	 ...
	 * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 */
	public void loadImage(String uri, ImageLoadingListener listener, String uid, int belong) {
		loadImage(uri, null, null, listener, uid, belong);
	}
	 ...
	 * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 */
	public void loadImage(String uri, ImageSize minImageSize, ImageLoadingListener listener, String uid, int belong) {
		loadImage(uri, minImageSize, null, listener, uid, belong);
	}
	 ...
	 * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 */
	public void loadImage(String uri, DisplayImageOptions options, ImageLoadingListener listener, String uid, int belong) {
		loadImage(uri, null, options, listener, uid, belong);
	}
	 ...
	 * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 */
	public void loadImage(String uri, ImageSize targetImageSize, DisplayImageOptions options, ImageLoadingListener listener, String uid, int belong) {
		checkConfiguration();
		...
		displayImage(uri, fakeImage, optionsWithFakeDisplayer, listener, uid,  belong);
	}
