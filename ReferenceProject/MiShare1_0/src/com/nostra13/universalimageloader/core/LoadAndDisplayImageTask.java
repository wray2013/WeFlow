...
final class LoadAndDisplayImageTask implements Runnable {
	...
	public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler, String uid, int belong) {
		this.engine = engine;
	...
	private Bitmap tryLoadBitmap(String uid, int belong) {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				...
				String imageUriForDecoding = options.isCacheOnDisc() ? tryCacheImageOnDisc(imageFile, uid, belong) : uri;
	...
	private File getImageFileInDiscCache() {
		...
		File imageFile = discCache.get(uri, uid);
		File cacheDir = imageFile.getParentFile();
		if (cacheDir == null || (!cacheDir.exists() && !cacheDir.mkdirs())) {
			imageFile = configuration.reserveDiscCache.get(uri, uid);
	...
	private String tryCacheImageOnDisc(File targetFile, String uid, int belong) {
		...
		try {
			...
			configuration.discCache.put(uri, targetFile, uid, belong);
