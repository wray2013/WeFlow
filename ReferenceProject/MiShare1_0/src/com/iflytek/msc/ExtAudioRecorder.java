...
/*	public int getMaxAmplitude() {
	 ...
	 * 
	 */
	public boolean start(String audioID, String path, boolean longAudio, int Belong, boolean useReg) {
		Log.d(TAG,"start in useReg = " + useReg);
		synchronized (state) {
			if(state==State.RECORDING){
		...
		TickUpHelper.getInstance(handler).init();
		
		sendRecDoneFlag = false;
		isRecording = true;
//		shortPath = path + "/" + audioID + "/" + audioID + ".mp4";
		shortPath = path + "/" + audioID + ".mp4";
...
//			handler.obtainMessage(HANDLER_AUDIO_RECORDER_VOLUME).sendToTarget();
				...
				if (handler != null) {
					handler.postDelayed(new Runnable() {
						...
						public void run() {
							...
							TickUpHelper.getInstance(handler).start(30);
						}
					}, 300);
				}
				synchronized (state) {
	 ...
	 * 
	 */
	public boolean start(String audioID, String path, boolean longAudio, int Belong) {
		return start(audioID, path, longAudio, Belong, false);
	}
	...
	/*				SimpleDateFormat formatter2 = new SimpleDateFormat("m:ss");//
					formatter2.setTimeZone(TimeZone.getTimeZone("GMT+00:00"));*/

					intent.putExtra("playtime", playtime/60000 + "'" + (playtime/1000)%60 + "''");
				}
				//double timeduration = XUtils.getMediaTotalTime(fullPath);
				Mp4InfoUtils tmp4info = new Mp4InfoUtils(fullPath);
				//audioduration = (long) (timeduration * 1000);
				audioduration = (long)(tmp4info.totaltime * 1000);
