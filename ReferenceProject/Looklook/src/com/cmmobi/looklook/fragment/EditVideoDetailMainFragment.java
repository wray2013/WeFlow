...
public class EditVideoDetailMainFragment extends Fragment implements OnClickListener, OnLongClickListener{

	private static final String TAG = "EditVideoDetailMainFragment";
	private EditVideoDetailActivity mActivity;
    private RelativeLayout mTagLayout = null;
    private RelativeLayout mPosLayout = null;
    private LinearLayout editVideoLayout = null;
    private RelativeLayout mBeautiSoundLayout = null;
...
//	private VolumeStateView vsv;.
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.fragment_edit_video_detail_main, container, false);
		mTagLayout = (RelativeLayout) view.findViewById(R.id.rl_edit_video_detail_tag);
		mTagLayout.setOnClickListener(this);
		
		mPosLayout = (RelativeLayout) view.findViewById(R.id.rl_edit_video_detail_position);
		mPosLayout.setOnClickListener(this);
		
		editVideoLayout = (LinearLayout) view.findViewById(R.id.ll_edit_video);
		editVideoLayout.setOnClickListener(this);
		...
		mActivity.positionView = (TextView) view.findViewById(R.id.tv_edit_video_position);
		...
		mActivity.tagTextView = (TextView) view.findViewById(R.id.tv_edit_video_detail_tag);
		mActivity.tagStr = "";
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch(v.getId()) {
		case R.id.rl_edit_video_detail_tag:
			if (mActivity.isLongPressed) {
				break;
			}
			mActivity.goToPage(EditVoiceDetailActivity.FRAGMENT_TAG_VIEW, false);
			break;
		case R.id.rl_edit_video_detail_position:
			if (mActivity.isLongPressed) {
				break;
			}
//			mActivity.hideVideoView();
//			mActivity.mainLayout.setVisibility(View.GONE);
//			mActivity.detailMainLayout.setVisibility(View.VISIBLE);
...
//				mActivity.mMediaPlayer.getXSurfaceView().setVisibility(View.GONE);
			}
			mActivity.hideVideoView();//rlVideoContent.removeAllViews();
		...
		case R.id.rl_edit_media_beautifysound:
			...
			break;
		case R.id.ll_edit_video:
			if (mActivity.isLongPressed) {
				break;
			}
			
			/*if (mActivity.mMediaPlayer.getStatus() == XEffectMediaPlayer.STATUS_PAUSE) {
				if (mActivity.videoMontageStartTime > 0.1) {
					mActivity.mMediaPlayer.seek(mActivity.videoMontageStartTime);
				}
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case SEND_SOUND_RECORD_MSG:
				...
				shortRecView.show(editVideoLayout);
				break;
			...
			case ExtAudioRecorder.HANDLER_AUDIO_RECORDER_DONE:
				Log.d(TAG,"HANDLER_AUDIO_RECORDER_DONE");
