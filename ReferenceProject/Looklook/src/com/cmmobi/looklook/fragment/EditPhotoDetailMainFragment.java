...
public class EditPhotoDetailMainFragment extends Fragment implements OnClickListener, OnLongClickListener{

	private static final String TAG = "EditPhotoDetailMainFragment";
    ...
    private LinearLayout editPhotoLayout = null;
...
//	private VolumeStateView vsv;.
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(R.layout.fragment_edit_photo_detail_main, container, false);
		mTagLayout = (RelativeLayout) view.findViewById(R.id.rl_edit_photo_detail_tag);
		mTagLayout.setOnClickListener(this);
		
		mPosLayout = (RelativeLayout) view.findViewById(R.id.rl_edit_photo_detail_position);
		...
		editPhotoLayout = (LinearLayout) view.findViewById(R.id.ll_edit_photo);
		editPhotoLayout.setOnClickListener(this);
		
		mActivity.positionView = (TextView) view.findViewById(R.id.tv_edit_photo_position);
		...
		mActivity.tagTextView = (TextView) view.findViewById(R.id.tv_edit_photo_detail_tag);
		mActivity.tagStr = "";
		...
		mActivity.mainLayout.setVisibility(View.INVISIBLE);
...
//						Log.e("isOnRecorderButton", isOnRecorderButton + "");
	...
	public void onClick(View v) {
		// TODO Auto-generated method stub
		switch(v.getId()) {
		case R.id.rl_edit_photo_detail_tag:
			if (mActivity.isLongPressed) {
				break;
			}
			mActivity.goToPage(EditVoiceDetailActivity.FRAGMENT_TAG_VIEW, false);
			break;
		case R.id.rl_edit_photo_detail_position:
			if (mActivity.isLongPressed) {
				break;
			}
		...
		case R.id.rl_edit_media_beautifysound:
			...
			if (mActivity.hasShortSound) {
				if (!EffectsDownloadUtil.getInstance(mActivity).checkEffects()) {
					...
					mActivity.goToPage(EditPhotoDetailActivity.FRAGMENT_SOUND_VIEW, false);
				} 
			}
			break;
		}
...
//				vsv.showVolume(editPhotoLayout);
				isRecording = true;
				recordDuration = 0;
				handler.sendEmptyMessage(HANDLER_RECORD_DURATION_UPDATE_MSG);
				shortRecView.show(editPhotoLayout);
				break;
