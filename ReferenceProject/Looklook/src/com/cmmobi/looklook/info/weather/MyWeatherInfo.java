...
//	private boolean isFailedLastTime = false;
	...
	private void init() {
	    ...
	    appFrontfilter.addAction(PrivateMessageService.FLAG_APP_FRONT_NOTIFY);
...
//		ci = CommonInfo.getInstance();
	...
	public void updateWeather(boolean isactive) {
		...
		if(myWeather != null) {
			Requester2.getWeather(handler, myWeather.addrCode);
		}
	}
	...
	public boolean handleMessage(Message msg) {
		...
		switch (msg.what) {
		case Requester2.RESPONSE_TYPE_GET_WEATHER:
...
//			Log.d(TAG, "lastdays = " + lastdays + ";today = " + today + ";delta = " + delta);
			...
			if(msg.obj!=null){
				GsonResponse2.getWeatherResponse response = (GsonResponse2.getWeatherResponse) msg.obj;
...
//						updateWeather(false);
	...
	private class AppFrontReceiver extends ZBroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (PrivateMessageService.FLAG_APP_FRONT_NOTIFY.equals(intent.getAction())) {
	...
	@Override
	public void onPoiSearch(List<String> res) {
		// TODO Auto-generated method stub
