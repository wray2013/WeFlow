 ...
 * 
 */
public class SettingBindPhoneActivity extends ZActivity implements TextWatcher{

	private Button btn_next;// 下一步
	private EditText et_phonenum;
	private ImageView iv_back;
	
	private final int REQUEST_CHECK_NO = 0x123401;
	...
	public void onClick(View v) {
		...
		case R.id.btn_next:
			...
			Requester2.getCheckNo(handler,et_phonenum.getText().toString().trim(), LoginSettingManager.GET_CHECK_NO_PHONE);
			break;
		case R.id.iv_back:
			this.onBackPressed();
			break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == REQUEST_CHECK_NO && resultCode == RESULT_OK){
			...
			try {
				...
				if (gsn.status.equals("0")) {
					Intent intent = new Intent(this,
							SettingBindPhoneChecknoActivity.class);
					intent.putExtra("phonenum", et_phonenum.getText().toString().trim());
					startActivityForResult(intent, REQUEST_CHECK_NO);	
				}else{
