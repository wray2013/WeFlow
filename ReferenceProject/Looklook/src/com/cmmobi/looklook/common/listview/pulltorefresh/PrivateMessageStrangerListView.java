 ...
 * @date  2013-6-26
 */
public class PrivateMessageStrangerListView extends AbsRefreshView<List<MessageWrapper>> {

	private static final String TAG="PrivateMessageStrangerListView";

	
	private Context context;
	public Handler handler;
	
	public PrivateMessageStrangerListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public PrivateMessageStrangerListView(Context context) {
		super(context);
		init(context);
	}
	...
	protected void addView(List<MessageWrapper> items) {
		...
		for(int i=0; i<items.size(); i++){
			...
			v = inflater.inflate(R.layout.list_item_stranger_message,null);
