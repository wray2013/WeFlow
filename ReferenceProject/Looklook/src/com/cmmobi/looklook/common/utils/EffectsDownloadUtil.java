...
//					Requester2.getEffects(handler, apkVersion);
					EffectLibDownloader downloader = new EffectLibDownloader(activity, true);
...
/*		if(apkVersion.length() == 3){
		...
		try{
			apkTmp = Long.parseLong(apkVersion.replace(".", "00000"));
			effectTmp = Long.parseLong(effectVersion.replace(".", "00000"));
...
/*	private boolean checkPlugin() {
	...
	private class DownloadEffectsAsync extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			if (Environment.getExternalStorageState().equals(
				...
				String url = params[0];
				File webimage_file = new File(filePath);
	...
	public boolean handleMessage(Message msg) {
		...
		case Requester2.RESPONSE_TYPE_GET_EFFECTS:
			GsonResponse2.getEffectResponse response = (GsonResponse2.getEffectResponse) msg.obj;
			if (response != null) {
