...
public class AsyncActiveAccountSaver extends Thread implements Callback{
	private static AsyncActiveAccountSaver  ins = null;
	private static final String TAG = "AsyncActiveAccountSaver";
	private Handler handler;
	private Class<?> cls;
	private Object saveObj;
	private String key;
	
	private final int HANDLER_FLAG_ASYNCSAVE_DONE = 0x18791713;
	private final int HANDLER_FLAG_SAVE_DELAY = 0x5381510;
	private transient final long TIME_DELAY_TO_SAVE = 3000; //3 秒
	
	private long requset_time;
	private long write_time;;
	
	public static AsyncActiveAccountSaver getInstance(){
		if(ins==null){
			ins = new AsyncActiveAccountSaver();
		}
	 ...
	 */
	private AsyncActiveAccountSaver() {
	...
	public void run(){
		while(true){
			if(write_time<requset_time){
				Log.e(TAG, "AsyncActiveAccountSaver - run, key: " + key + " persist!");
				int tryCount = 3;
				while(tryCount>0){
					try{
