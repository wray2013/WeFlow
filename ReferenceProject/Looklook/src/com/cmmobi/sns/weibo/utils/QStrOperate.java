package com.cmmobi.sns.weibo.utils;
...
public class QStrOperate {
    ...
    public static String getTimeState(String timestamp) {
        ...
        try {
            ...
            if (TimeHelper.getInstance().now() - _timestamp < 1 * 60 * 1000) {
                return "刚刚";
            } else if (TimeHelper.getInstance().now() - _timestamp < 30 * 60 * 1000) {
                return ((TimeHelper.getInstance().now() - _timestamp) / 1000 / 60)
                        + "分钟前";
            } else {
