...
public class CmmobiSnsLib
{
	...
	public void updateOAuthV2Map(int curWeiboIndex, OAuthV2 oAuth) {
		...
		if(aa.isLogin()) {  //将oAuthV2Map 同步到accountInfo
			AccountInfo.getInstance(aa.getLookLookID()).updateAccessToken(new GsonResponse2.MyBind(curWeiboIndex, oAuth));
		}
	}
...
*****************************************************************************************************/
	...
	public String sinaUploadUrlText(String status, String imageUrl, String lat, String lon) throws Exception {
		...
		if(jsonResult != null){
			WeiboError tencentWeiboError = JsonParseUtils.parseSinaWeiboError(jsonResult);
	...
	public boolean sinaDel(String weiboID) throws Exception {
		...
		if(jsonResult != null){
			WeiboError sinaWeiboError = JsonParseUtils.parseSinaWeiboError(jsonResult);
			if(sinaWeiboError != null){
				return false;
			}else{
				return true;
			}
...
*/
	...
	public String twUploadUrlText(String content,
		...
		if(jsonResult != null){
			WeiboError tencentWeiboError = JsonParseUtils.parseTencentWeiboError(jsonResult);
			if(tencentWeiboError != null){
				return null;
			}else{
...
/*				Toast.makeText(CmmobiSnsLib.this.mContext, "登录成功",
	 ...
	 * @throws Exception 
	 */
	public String commentOnRW(String snsid, int id,String comment) throws Exception {
		if(null == this.mRWCommentsApi){
			mRWCommentsApi = new RWCommentsAPI(OAuthConstants.OAUTH_VERSION_2_A);
		}
		return mRWCommentsApi.reply(acquireRenrenOauth(), id, snsid, comment);
	}
